...
-   **在线阅读** - 支持 EPUB/TXT 格式的章节阅读
-   **章节识别** - 自动识别中文章节标题（第X章/节/回）
-   **章节导航** - 按章节浏览，返回章节列表和内容
-   **文件下载** - 支持下载所有格式的电子书（PDF/MOBI/AZW/FB2 仅支持下载）
-   **封面显示** - 无封面时自动生成标题占位图
-   **阅读进度** - 记录当前阅读位置，支持续读
...
| 类型 | 格式 | 扫描入库 | 在线阅读 | 说明 |
|------|------|---------|---------|------|
| **音频** | MP3, FLAC, OGG, WAV, AAC, M4A | ✅ | ✅ 流媒体播放 | 支持 Range 请求，断点续播 |
| **视频** | MP4, MKV, AVI, MOV, FLV, WMV, WebM, M4V | ✅ | ✅ 流媒体播放 | 支持 Range 请求，断点续播 |
| **漫画** | CBZ | ✅ | ✅ 按页浏览 | ZIP 格式压缩包 |
| | CBR | ✅ | ✅ 按页浏览 | RAR 格式压缩包 |
| | ZIP | ✅ | ✅ 按页浏览 | 与 CBZ 相同处理 |
| | RAR | ✅ | ⚠️ 部分支持 | 依赖 Tika，某些 RAR5 可能有问题 |
| | EPUB | ✅ | ✅ 按页浏览 | 可作为漫画格式读取 |
| **电子书** | EPUB | ✅ | ✅ 章节阅读 | 完整支持，含内嵌图片 |
| | TXT | ✅ | ✅ 章节阅读 | 自动识别中文章节标题 |
| | PDF | ✅ | ❌ 仅下载 | 仅支持下载，不支持在线阅读 |
| | MOBI | ✅ | ❌ 仅下载 | 仅支持下载，不支持在线阅读 |
| | AZW/AZW3 | ✅ | ❌ 仅下载 | 仅支持下载，不支持在线阅读 |
| | FB2 | ✅ | ❌ 仅下载 | 仅支持下载，不支持在线阅读 |
