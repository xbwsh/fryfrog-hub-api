...
### 1.5.9 查看用户可访问媒体库（管理员）
- **GET** `/users/{id}/libraries`
- **响应**: `ApiResponse<List<Long>>`（分配的媒体库 ID 列表）

### 1.5.10 分配用户可访问媒体库（管理员，幂等替换）
- **PUT** `/users/{id}/libraries`
- **请求体**:
```json
{
  "libraryIds": [1, 2, 3]
...
```
- 传空数组表示收回全部授权；效果：普通用户仅能看到「被分配 ∩ 启用」的媒体库内容，ADMIN 无限制。
